"use strict";
const satellites_1 = require('./lib/satellites');
const fs = require('fs');
if (process.argv.length !== 3)
    throw "Please supply a data file.";
...
let constellation = new satellites_1.SatelliteConstellation();
data.forEach(line => {
    let satelliteData = line.match(/(\w+),(-?\d+\.\d+),(-?\d+\.\d+),(\d+\.\d+)/);
    constellation.addSatellite(new satellites_1.Satellite(satelliteData[1], { latitude: parseFloat(satelliteData[2]), longitude: parseFloat(satelliteData[3]) }, parseFloat(satelliteData[4])));
});
